...
import { format_time_ago, extract_thumbnail, } from "@kkocot/honeycomb-core";
/**
 ...
 */
function parse_asset_string(asset) {
    if (!asset)
        return 0;
    const match = asset.match(/^([\d.]+)/);
    return match ? parseFloat(match[1]) : 0;
}
function is_valid_post_result(data) {
 ...
 * ```
 */
export function useHivePost(author_or_getter, permlink_or_getter) {
    const ctx = useHive();
    const get_author = typeof author_or_getter === "function"
        ? author_or_getter
        : () => author_or_getter;
    const get_permlink = typeof permlink_or_getter === "function"
        ? permlink_or_getter
        : () => permlink_or_getter;
    let post = $state(null);
        ...
        async function fetch_post() {
            try {
                ...
                if (is_valid_post_result(data)) {
                    ...
                    const pending_payout = parse_asset_string(comment.pending_payout_value);
                    const author_payout = parse_asset_string(comment.total_payout_value);
                    const curator_payout = parse_asset_string(comment.curator_payout_value);
                    ...
                    post = {
                        ...
                        thumbnail: extract_thumbnail(comment.json_metadata || "{}", comment.body) ?? `https://images.hive.blog/u/${comment.author}/avatar`,
                    };
                }
                else {
