...
import { fetch_bridge_reputation, format_nai_asset, convert_vests_to_hp, } from "@kkocot/honeycomb-core";
function format_with_symbol(amount, symbol) {
 ...
 * ```
 */
export function useHiveAccount(username_or_getter) {
    const ctx = useHive();
    const get_username = typeof username_or_getter === "function"
        ? username_or_getter
        : () => username_or_getter;
    ...
    $effect(() => {
        ...
        async function fetch_account() {
            try {
                const [account_response, global_response, reputation] = await Promise.all([
                ...
                if (acc) {
                    ...
                    const hp = convert_vests_to_hp(acc.vesting_shares, global_response.total_vesting_shares, global_response.total_vesting_fund_hive);
                    const delegated_hp_value = convert_vests_to_hp(acc.delegated_vesting_shares, global_response.total_vesting_shares, global_response.total_vesting_fund_hive);
                    const received_hp_value = convert_vests_to_hp(acc.received_vesting_shares, global_response.total_vesting_shares, global_response.total_vesting_fund_hive);
