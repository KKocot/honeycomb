...
export class Phishing {
  public static looks_phishy = (questionable_url: string): boolean => {
    const lowercase_url = questionable_url.toLowerCase();
    for (const domain of domains) {
      if (lowercase_url.indexOf(domain) > -1) return true;
    }
    return false;
  };
}
