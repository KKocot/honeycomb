...
import type { PostHideOption } from "./post-card.js";
import {
  should_hide,
...
export function render_loading(class_name: string | undefined): VNode {
  return h(
    "div",
    {
      class: cn(
...
export function render_compact(
  post_url: string,
  author: string,
  post: {
    title: string;
  ...
  if (!should_hide(hide, "payout")) {
    stats.push(h("span", { class: "text-hive-success" }, post.payout));
  }
  if (stats.length > 0) {
    content_children.push(
      h(
...
export function render_default_card(
  post_url: string,
  author: string,
  post: {
    title: string;
