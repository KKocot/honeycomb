...
const TABS: readonly TabConfig[] = [
  ...
  },
  {
    ...
    description:
      "Paginated post feed for a specific Hive author with sort controls, tag filtering, and multiple layouts.",
  },
  {
    id: "renderer",
    label: "Renderer",
...
export function meta() {
  return [{ title: "Honeycomb React Router Demo" }];
}

export default function Home() {
  const [search_params, set_search_params] = useSearchParams();
  const tab_from_url = search_params.get("tab");
  const active_tab = is_valid_tab(tab_from_url) ? tab_from_url : DEFAULT_TAB;

  const handle_tab_change = (tab_id: TabId) => {
    set_search_params({ tab: tab_id });
  };
