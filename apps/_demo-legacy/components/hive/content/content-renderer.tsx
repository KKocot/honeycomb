...
// Local plugins
import { remarkInternalLinks, remarkSubSup, rehypeLinkHandler } from "./plugins";

// Components
import LinkHeader from "./link-header";
import MermaidComponent from "./mermaid-component";
import { LeavePageDialog } from "./leave-page-dialog";
import { getDoubleSize, proxifyImageUrl } from "./image-proxy";
...
/**
 * Renders Hive blockchain markdown content with support for:
 * - Embeds (YouTube, Twitch, Instagram, Twitter/X, 3Speak)
 * - Mermaid diagrams
 * - Math equations (KaTeX)
 * - Code syntax highlighting
 * - Internal @mentions and #tags
 * - Safe/dangerous link handling
 */
export function HiveContentRenderer({
  ...
  return (
    <div
      className={cn(
        "w-full relative min-w-full self-center break-words p-2 font-source text-[16.5px] prose-h1:text-[26.4px] prose-h2:text-[23.1px] prose-h3:text-[19.8px] prose-h4:text-[18.1px] sm:text-[17.6px] sm:prose-h1:text-[28px] sm:prose-h2:text-[24.7px] sm:prose-h3:text-[22.1px] sm:prose-h4:text-[19.4px] lg:text-[19.2px] lg:prose-h1:text-[30.7px] lg:prose-h2:text-[28.9px] lg:prose-h3:text-[23px] lg:prose-h4:text-[21.1px] prose-p:mb-6 prose-p:mt-0 prose-img:cursor-pointer prose max-w-none dark:prose-invert",
        className
      )}
    >
      <ReactMarkdown
...
const LinkComponent = ({
  ...
  const youtube = getYoutubeFromLink(url);
  if (youtube) {
    return (
      <div
