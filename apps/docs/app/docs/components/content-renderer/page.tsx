...
import { ArrowLeft, ArrowRight, Info, Youtube, MessageSquare, Image, Code, Link2, AtSign } from "lucide-react";
...
const CODE = {
  ...
  basic: `import { HiveContentRenderer } from "@/components/hive/content";

function PostContent({ body }: { body: string }) {
  return <HiveContentRenderer content={body} />;
}`,
  withClassName: `<HiveContentRenderer
  content={postBody}
  className="max-w-3xl mx-auto"
/>`,
  fetchPost: `// Fetch post and render content
...
export default async function ContentRendererPage() {
  return (
    <article className="space-y-8">
      <div>
        <h1 className="text-3xl font-bold tracking-tight">HiveContentRenderer</h1>
        <p className="mt-2 text-lg text-muted-foreground">
          Renders Hive blockchain markdown content with full embed and formatting support.
        </p>
      </div>
      ...
      <section className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        <div className="rounded-lg border border-border p-4">
          ...
          <p className="text-sm text-muted-foreground">YouTube, Twitch, 3Speak</p>
        </div>
        <div className="rounded-lg border border-border p-4">
        ...
        <div className="rounded-lg border border-border p-4">
          ...
          <p className="text-sm text-muted-foreground">Syntax highlighting for 180+ languages</p>
        </div>
        <div className="rounded-lg border border-border p-4">
          <Image className="h-5 w-5 text-hive-red mb-2" />
          <h3 className="font-medium">Image Proxy</h3>
          <p className="text-sm text-muted-foreground">Auto-proxied via images.hive.blog</p>
        </div>
        <div className="rounded-lg border border-border p-4">
          <AtSign className="h-5 w-5 text-hive-red mb-2" />
          <h3 className="font-medium">@mentions & #tags</h3>
          <p className="text-sm text-muted-foreground">Auto-linked to profiles and tags</p>
        </div>
        <div className="rounded-lg border border-border p-4">
          <Link2 className="h-5 w-5 text-hive-red mb-2" />
          <h3 className="font-medium">Safe Links</h3>
          <p className="text-sm text-muted-foreground">Phishing protection built-in</p>
        </div>
      </section>
      ...
      <section className="rounded-lg border border-blue-500/20 bg-blue-500/5 p-4">
        <div className="flex gap-3">
          ...
          <div>
            <p className="font-medium text-blue-500">Full Hive Compatibility</p>
            <p className="mt-1 text-sm text-muted-foreground">
              Supports all Hive markdown extensions including flexible markers,
              GitHub-style alerts, math equations (KaTeX), Mermaid diagrams, and more.
            </p>
          </div>
        </div>
      </section>
          ...
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-border">
              <tr>
                ...
                <td className="py-3 px-4 text-muted-foreground">Markdown content to render</td>
              </tr>
              <tr>
      ...
      <section>
        ...
        <div className="space-y-4">
          <div>
            <h3 className="text-sm font-medium mb-2">Video & Social Embeds</h3>
      ...
      <section>
        ...
        <div className="space-y-6">
          <div>
            <h3 className="text-sm font-medium mb-2">With custom className</h3>
            <CodeBlock code={CODE.withClassName} language="tsx" />
          </div>
          <div>
            <h3 className="text-sm font-medium mb-2">Fetch and render post</h3>
            <CodeBlock code={CODE.fetchPost} language="tsx" />
          </div>
        </div>
      </section>
      ...
      <section className="flex items-center justify-between">
        ...
        <Link
          href="/docs/components/markdown-editor"
